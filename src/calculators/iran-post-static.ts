...
export class IranPostStaticCalculator implements IShippingCalculator {
  ...
  private getDistanceType(origin: string, destination: string): "intra" | "adjacent" | "non_adjacent" {
    if (!origin || !destination) return "non_adjacent"
    ...
    if (originAdjacents.includes(d) || destAdjacents.includes(o)) return "adjacent"

    // Default fallback
    return "non_adjacent"
  }

  private getCoastalSurcharge(destination: string, weightInKg: number): number {
  ...
  private getInsuranceCost(cartTotalRials: number): number {
    // Returns insurance cost in Toman
    if (cartTotalRials <= 0) return 10000 // default minimum 10,000 Toman
  ...
  public async calculate(
    ...
    const destinationProvince = (context.shipping_address?.province as string) || ""
    const destinationCity = (context.shipping_address?.city as string) || ""
  ...
  private calculatePishtaz(weight: number, distance: "intra" | "adjacent" | "non_adjacent"): number {
    // Prices in Toman
    if (weight <= 0.5) {
      if (distance === "intra") return 70000
      if (distance === "adjacent") return 80000
      return 90000
    }
    
    if (weight <= 1) {
      if (distance === "intra") return 80000
      if (distance === "adjacent") return 90000
      return 100000
    }
  ...
  private calculateSpecial(weight: number, distance: "intra" | "adjacent" | "non_adjacent"): number {
  ...
  private calculateExpress(weight: number, distance: "intra" | "adjacent" | "non_adjacent"): number {
  ...
  private calculateSefareshi(weight: number, distance: "intra" | "adjacent" | "non_adjacent"): number {
    // Prices in Toman. Adding fixed SMS and ID fee of 7,700 Toman to the base logic
    const smsAndIdFee = 7700
    let base = 0

    if (weight <= 0.5) {
      if (distance === "intra") base = 15500
      else if (distance === "adjacent") base = 20500
      else base = 22900
    }
    else if (weight <= 1) {
      if (distance === "intra") base = 20500
      else if (distance === "adjacent") base = 28500
      else base = 32000
    }
    else if (weight <= 2) {
      if (distance === "intra") base = 29000
      else if (distance === "adjacent") base = 37000
      else base = 41200
    }
    else if (weight <= 3) {
      if (distance === "intra") base = 32000
      else if (distance === "adjacent") base = 38500
      else base = 44900
    }
    else if (weight <= 5) {
      if (distance === "intra") base = 27000 // wait, table 1 row 7 says 270,000. It's actually cheaper than 2-3kg? Yes, likely subsidized for large parcels.
      else if (distance === "adjacent") base = 45500
      else base = 52500
    }
    else {
